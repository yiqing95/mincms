		...
		<?php }else{?>
			<?php if($post['active'] == 1){?>  
				<span class="label label-success">
					<a class='ajax_add'  url="<?php echo url('admin/modules/install',array(
					'id'=>$post['name'].'.uninstall'));?>" rel=1 id="<?php echo $post['name'];?>"><i class="icon-trash icon-white"></i></a> </span>
			<?php }else{?>
			<a class='ajax_add' id="<?php echo $post['name'];?>" url="<?php echo url('admin/modules/install',array('id'=>$post['name'].'.install'));?>"><i class="icon-plus"></i></a>
			<?php }?>
		<?php }?>
	</td>
...
js(" 
	$('.ajax_add').click(function(){
		...
		$.post('".url('admin/modules/add')."', { id: id ,YII_CSRF_TOKEN:'".Yii::$app->request->csrfToken."'},
		function(data) { 
			 if(data==1){
			 	$(j).html(html);
			 	$('.ajax_ok').fadeOut();  
			 	$.get('".url('admin/modules/index')."',
				function(data){ 
				 	$('body').html(data);  
